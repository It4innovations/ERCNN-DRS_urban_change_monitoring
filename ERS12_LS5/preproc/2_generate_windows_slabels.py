...
tf_record_out_path = "/data/data_temp/training_ls5ers12/"
...
min_obs = 25 # how many observations per window we need at least
...
def _parse_tfr_element(element):
    parse_dic = {
        ...
        "ERS12_ascending": tf.io.FixedLenFeature([], tf.string),
        "ERS12_descending": tf.io.FixedLenFeature([], tf.string),
        "LS5": tf.io.FixedLenFeature([], tf.string)
    }
    ...
    ers12_ascending_f = example_message["ERS12_ascending"]
    ers12_ascending = tf.ensure_shape(tf.io.parse_tensor(ers12_ascending_f,
                                                         out_type=tf.float32),
                                     [tile_size_y, tile_size_x, 1])

    ers12_descending_f = example_message["ERS12_descending"]
    ers12_descending = tf.ensure_shape(tf.io.parse_tensor(ers12_descending_f,
                                                          out_type=tf.float32),
                                       [tile_size_y, tile_size_x, 1])

    ls5_f = example_message["LS5"]
    ls5 = tf.ensure_shape(tf.io.parse_tensor(ls5_f,
                                             out_type=tf.float32),
                          [tile_size_y, tile_size_x, 7])

    return timestamp, ers12_ascending, ers12_descending, ls5

def sub_to_batch(timestamp, ers12_ascending, ers12_descending, ls5):
    def first_only(batch):
        return tf.broadcast_to(batch[0], [tf.size(batch)])

    sub_ls5 = zip(timestamp, ls5)
    ref = timestamp.batch(buffer_size * 3).map(first_only)

    # 1st period (LS5 only)
    batch_1st = sub_ls5.batch(buffer_size * 3)
    ...
    # 3rd period (LS5 only)
    batch_3rd = sub_ls5.batch(buffer_size * 3)
    ...
    sub = zip(timestamp, ers12_ascending, ers12_descending, ls5)
...
def stack_layers(stack, ls5_1, ls5_3):
    return tf.concat([
                stack[1],
                stack[2],
                stack[3]], axis=-1)
...
def annotate_ds(file, shift, seed, just_timestamps = False,
    ...
    def generate_label(first_stack, betas):
        ...
        return tf.ensure_shape(tf.numpy_function(
                                    ...
                                    [stack[1], stack[2], stack[3], ls5_1, ls5_3,
