...
base_dir = "/data_temp/training_ls5ers12_random_windows_scale_30/"
...
time_range = 60*60*24*365 # Window observation period (in seconds) (LS5: 365 days)
...
if combined:
    ...
    if sar:
        train_ds = train_ds.map(lambda x, y: (x[:, :, :, 0:2], y))
    else:
        train_ds = train_ds.map(lambda x, y: (x[:, :, :, 2:], y))
...
if combined:
    ...
    if sar:
        val_ds = val_ds.map(lambda x, y: (x[:, :, :, 0:2], y))
    else:
        val_ds = val_ds.map(lambda x, y: (x[:, :, :, 2:], y))
